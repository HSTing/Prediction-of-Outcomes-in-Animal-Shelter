...
def primaryColor(var):
    ...
    :return: new color
    """
    color = ['BLACK', 'GRAY', 'BROWN', 'ORANGE', 'WHITE', 'CALICO', 'TORTIE', 'BUFF', 'CREAM', 'TAN', 'SILVER',
             'SEAL', 'BLUE', 'LYNX', 'CHOCOLATE', 'FLAME', 'YELLOW', 'TRICOLOR']
    var = var.split()
    if len(var) > 1:
        mark = 'mark'
    else:
...
def primaryBreed(var):
    ...
    :return: new breed
    """
    breed = ['DOMESTIC SHORTHAIR', 'DOMESTIC MEDIUMHAIR', 'DOMESTIC LONGHAIR', 'AMERICAN SHORTHAIR', 'SIAMESE',
             'MAINE COON', 'RUSSIAN BLUE', 'BENGAL', 'PERSIAN', 'SNOWSHOE', 'MANX', 'HIMALAYAN']
    if var in breed:
        return var
    else:
        return 'OTHER'
...
def main():
    ...
    with open('CAT_data.csv', "w") as csv_file:
